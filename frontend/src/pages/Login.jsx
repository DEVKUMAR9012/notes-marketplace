          ...
          <AnimatePresence mode="wait">
            {step === 'login' ? (
              ...
              >
                ...
                {/* ── Google One-Tap alternative (only shown when Client ID is configured) ── */}
                {process.env.REACT_APP_GOOGLE_CLIENT_ID?.endsWith('.apps.googleusercontent.com') && (
                  <>
                    <div className="flex items-center gap-3 mt-6">
                      <div className="flex-1 h-px bg-white/8" />
                      <span className="text-xs text-gray-600 font-medium tracking-widest uppercase">Or continue with</span>
                      <div className="flex-1 h-px bg-white/8" />
                    </div>

                    {/* Renders Google's native One-Tap overlay automatically */}
                    <NativeSocialLogins />

                    {/* Fallback visible button for users who dismissed the prompt */}
                    <button
                      type="button"
                      ...
                      className="w-full mt-3 flex items-center justify-center gap-3 py-3 px-4 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 hover:border-white/20 transition-all group"
                    >
                      {/* Google's official coloured G icon */}
                      ...
                      <span className="text-sm font-semibold text-gray-300 group-hover:text-white transition">Continue with Google</span>
                    </button>
                  </>
                )}
